...
namespace DreamEchoesCore.Utils
{
    ...
    /// </summary>
	public class WeaverCoreDreamWrapperOverride : DreamnailableObject
    {
        [SerializeField]
        ...
        protected override int OnDreamnailHit(Player player)
        {
            if (CanBeDreamnailed)
            {
                WeaverCoreWarpOverride.DoDreamnailWarp(transform.position, destinationScene, returnScene, transitionGateName, warpDelay, !canUseCharms);
            }
